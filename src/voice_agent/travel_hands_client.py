...
def get_auth_token(session=None):
    ...
    if session and hasattr(session, 'auth_token') and session.auth_token:
        ...
        logging.info("Token repr: %r", session.auth_token)
...
def get_existing_addresses(session,user_id):
    ...
    try:
        ...
        logging.info(f"🔑 Auth token for get_existing_addresses API call: {auth_token}")
