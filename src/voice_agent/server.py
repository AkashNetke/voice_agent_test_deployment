...
from src.voice_agent import test_booking_service
...
# Suppress verbose Azure Cosmos DB logging
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("✅ Enhanced LangGraph Booking Agent initialized directly")
    except Exception as e:
        logger.error(f"❌ Enhanced LangGraph Booking Agent initialization failed: {str(e)}")
        raise
...
def voice_agent(payload: Request = Body(...)):
    ...
    try:
        ...
        logger.error(f"Enhanced LangGraph agent processing failed: {str(e)}")
...
def voice_agent_v3(payload: Request = Body(...)):
    """
...
# ADDED FOR TRAVEL HANDS TESTING PURPOSES ONLY
@app.post("/feature/test", response_model=Response)
def voice_agent(payload: Request = Body(...)):
    """
    Main voice agent endpoint
    Processes voice messages and returns voice responses
   ...
   # ✅ Handle greeting / session initialization
    if payload.type == MessageType.REQUEST_GREETING:
        booking_service = test_booking_service.TestBookingService()
        greeting_text = booking_service.initialize_session(session)
        response_audio = app_state.audio_processor.text_to_speech_base64(greeting_text, "general")

        return Response(
            type=MessageType.AGENT_VOICE_MESSAGE,
            text_data=greeting_text,
            audio_data=response_audio
        )
    ...
    if not session.greeting_shown:
        booking_service = test_booking_service.TestBookingService()
        greeting_message = booking_service.initialize_session(session)
        logger.info(f"Session initialized with greeting: {greeting_message}")
    ...
    try:
        booking_service = test_booking_service.TestBookingService()
        response_text, is_complete = booking_service.process_user_input(
            session, user_text
        )
        logger.info(f"Journey booking response: {response_text[:100]}...")
    ...
    return Response(
        # type=MessageType.AGENT_VOICE_MESSAGE,
        type=MessageType.AGENT_VOICE_MESSAGE if payload.type == MessageType.USER_VOICE_MESSAGE else MessageType.AGENT_TEXT_MESSAGE,
        text_data=response_text,
        audio_data=response_audio
    )
