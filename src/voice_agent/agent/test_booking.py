...
def request_journey_api( journey_data, selected_volunteer,session):
    try:
        payload = {
            ...
            "requestDate": journey_data.get("journey_date"),
            "journeyEndTime": journey_data.get("journey_end_time", ""),
            "journeyNote": journey_data.get("journey_note", "Created via voice assistant"),
            "journeyReason": journey_data.get("journey_reason"),
            "pickupAddressId": journey_data.get("pickup_address_id"),
            "requestTime": journey_data.get("pickup_time"),
            "totalTimeForVolunteer": journey_data.get("total_time_volunteer", "")
        }

        endpoint = f"{TRAVEL_HANDS_API_BASE_URL}/api/vip/requestJourney/{session.user_id}"
        ...
        logging.info(f"Request Journey Response {response.status_code}: {response.text}")
    ...
    # Added by Akash Netke
def get_route_summary_from_api(pickup_address, dest_address, token):
    """
    Fetches a readable route summary from the Spring Boot /routes endpoint.
    ...
    try:
        # ✅ Base URL (replace with your actual deployment endpoint)
        ROUTE_API_BASE_URL = f"{TRAVEL_HANDS_API_BASE_URL}/api/vip/routes"

        # ✅ Prepare headers (same as other authenticated calls)
        headers = {
        ...
        # ✅ Log request details
        logger.info("=" * 80)
        logger.info("FETCHING PUBLIC TRANSPORT ROUTE FROM SPRING BOOT API")
        logger.info("=" * 80)
        logger.info(f"Endpoint: {ROUTE_API_BASE_URL}")
        logger.info(f"From: {pickup_address}")
        logger.info(f"To: {dest_address}")

        # ✅ Send GET request with from/to params
        response = requests.get(
            ROUTE_API_BASE_URL,
            params={"from": pickup_address, "to": dest_address},
            headers=headers,
            timeout=60
        )
        ...
        if response.status_code in [200, 201]:
            return {
                "success": True,
                "route_summary": response.text.strip(),
            ...
            return {
                "success": False,
                "route_summary": "",
                "message": f"Route API failed with status {response.status_code}"
            }
        ...
        return {
            "success": False,
            "route_summary": "",
            "message": "Request timeout while fetching route details"
        }
    except requests.exceptions.RequestException as e:
        return {
            "success": False,
            "route_summary": "",
            "message": f"Network error while calling route API: {str(e)}"
        }
    except Exception as e:
        return {
            "success": False,
            "route_summary": "",
            "message": f"Unexpected error while fetching route: {str(e)}"
        }
